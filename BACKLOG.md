...
## v1.1 implementation session â€” June 9, 2026 (cloud, no device)

### Shipped on branch claude/friendly-hypatia-qd9kxp
- Reliability pass: ActionRouter top-level guard â€” every route now ends in
...
- ROOT-CAUSE FIX: `https` was missing from manifest <queries> â€” canLaunchUrl()
  could silently return false on Android 11+ for wa.me drafts + every web
  fallback. Likely the WhatsApp draft flakiness.
- Grocery-no-app path: dead grofers:// â†’ verified https app-link for Blinkit;
  Zepto + BigBasket https templates added (UNVERIFIED â€” probe first);
...
- Explicit-Zomato degraded honestly: "Zomato khol raha hoon â€” X wahan khud
...
- Play audit: docs/play-paperwork.md â€” MANAGE_EXTERNAL_STORAGE flagged as the
...
- Fixed wrong Blinkit package in VaniPackages (com.blinkit.consumer â†’
  com.grofers.customerapp) â€” apps screen installed-check was broken.
- Offline STT (Vosk): full recipe in docs/offline-stt-vosk.md â€” deliberately
  NOT wired in from a machine that can't compile-check; one device evening.

### Before merge (device session â€” docs/v1.1-device-checklist.md)
...
3. Zomato dumpsys deep-link hunt (optional â€” honest copy ships either way)
